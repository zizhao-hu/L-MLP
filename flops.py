...
def evaluate(config):
    ...
    dummy_input = torch.randn(1, 4, 32, 32, dtype=torch.float16, device = device)  # example for an input image of size 32x32 with 3 channels
    timesteps = torch.ones(1, device=device) # example timestep value
    _empty_context = torch.randn(1, 77, 768, device = device)  # example context as an empty dictionary

    # Adjusted input_constructor that now correctly accepts an input resolution argument
    def input_constructor(input_res):
        return {'x': dummy_input, 'timesteps': timesteps, 'context': _empty_context}

    # Calculate FLOPs
    flops, params = get_model_complexity_info(nnet, (4, 32, 32), as_strings=True,
